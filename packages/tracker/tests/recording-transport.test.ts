...
describe("sendRecordingBatch", () => {
  ...
  it("posts to <recordEndpoint>/<siteId>/<sessionId> with a bearer token and the batch body", () => {
    sendRecordingBatch("https://recorder.example.test/recordings", "site-1", "session-1", "wtoken", 3, [{ a: 1 }], false);
  ...
  it("passes keepalive through as given by the caller", () => {
    sendRecordingBatch("https://recorder.example.test/recordings", "site-1", "session-1", "wtoken", 3, [{ a: 1 }], false);
    expect(vi.mocked(fetch).mock.calls[0][1]?.keepalive).toBe(false);

    sendRecordingBatch("https://recorder.example.test/recordings", "site-1", "session-1", "wtoken", 4, [{ a: 1 }], true);
    expect(vi.mocked(fetch).mock.calls[1][1]?.keepalive).toBe(true);
  });

  it("strips a trailing slash from recordEndpoint before joining the path", () => {
    sendRecordingBatch("https://recorder.example.test/recordings/", "site-1", "session-1", "wtoken", 0, [], false);
  ...
  it("never throws when fetch rejects", async () => {
    ...
    expect(() => sendRecordingBatch("https://recorder.example.test/recordings", "s", "sess", "t", 0, [], false)).not.toThrow();
  });
});
