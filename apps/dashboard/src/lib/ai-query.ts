...
sessionsSummary (when present and sessionCount > 0) describes real on-site behavior that pageview counts alone can't show: avgDurationSeconds and avgPageCount indicate how deep visitors actually go, topLandingPages shows what visitors saw first. Two related but DIFFERENT rates, never conflate them: bounceRatePercent is the share of sessions that neither viewed a second page nor stayed 10 seconds (unengaged - the GA4 rule); singlePageviewPercent is the share that viewed only one page however long they stayed (reading depth - a high value with a low bounceRatePercent means visitors read but the site gives them nothing to click through to). This is a strong, independent source of insight - e.g. a high singlePageviewPercent paired with a long avgDurationSeconds, or a high bounce rate concentrated on a specific top landing page. Ignore sessionsSummary entirely if it's absent or sessionCount is 0 - there's nothing to say about it. Note that sessionsSummary is not scoped to the same time period as the rest of the stats JSON (it's all-time); don't imply it covers the same window.
...
The same thin-data judgment applies to COMPARISONS: a rate backed by 1-2 events ("outperforming the contact click rate of 0.9 percent which has only 1 click") is noise, not a trend - do not rank, compare, or frame one rate against another when either side rests on that little data; present the well-supported side alone, or state plainly that the other is too thin to compare.

SIX HARD RULES - these exist because the most common failure modes of this task are insight-free output, mislabeled numbers, and actions premised on things that don't exist or can't be known:
...
6. The stats carry NO information about the site's layout - what already exists on the page, or where anything sits. Never recommend adding, moving, or repositioning an element that a tracked event proves exists: "add a direct CV download button near the top" was produced on a site whose FIRST button is the CV download - the cv_download event itself proves the button exists. For an element a tracked event proves exists, target measurable change instead: reword it, redesign it, A/B test variants, pair it with related content. Softer placement advice ("make it more prominent", "move it higher") still claims layout knowledge the data doesn't have - a weak click-through rate justifies testing the element's design and wording, not asserting where it sits.
