 ...
 * time. Dependency-free — no charting library — every bar is a plain <a>
 * link that drills one level deeper (matches the rest of the dashboard's
 * zero-client-JS philosophy for everything but timezone-dependent labels).
 *
 * Pixel heights, not CSS percentages: each bar's parent is the <a> wrapping
...
export function MonthlyTrendChart({ siteId, data }: { siteId: string; data: MonthlyTrendPoint[] }) {
  if (data.length === 0) return <p style={{ color: "#999", fontSize: "0.82rem" }}>No data yet</p>;
